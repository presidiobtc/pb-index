(function (root, factory) {
  ...
  function initialView(snapshot, query) {
    if (snapshot && typeof snapshot === "object" &&
        typeof snapshot.query === "string" && snapshot.query.trim() &&
        typeof snapshot.answer === "string" && snapshot.answer.trim() && snapshotPath(snapshot)) {
      return { kind: "snapshot", data: snapshot };
    }
    const cleanQuery = String(query || "").trim();
    if (cleanQuery.length >= 3) return { kind: "query", query: cleanQuery };
    return { kind: "empty" };
  }

  return { applySnapshotUrl, canonicalUrl, initialView, shareUrl, snapshotPath };
});
