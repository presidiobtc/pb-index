...
test("the checked-in topic artifact normalizes to a complete reusable dataset", () => {
  ...
  assert.equal(data.metadata.display_episode_count, 83);
  assert.equal(data.metadata.expected_episode_count, 81);
  // The measured total now happens to match the independent reference total.
  assert.equal(data.metadata.episode_count, 81);
...
test("the mounted explorer renders both Overview and the full quarterly Trends view", async () => {
  ...
  assert.match(root.innerHTML, /<strong>Based on 83 full PBJ episodes<\/strong>/);
  assert.doesNotMatch(
    root.innerHTML,
